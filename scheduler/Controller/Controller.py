    ...
    def makeSchedule(self):
        ...
        for i in range(len(self.courses[0].instructors)):
            for j in range(len(self.courses[0].instructors[i].groups)):
                tutsNum = len(self.courses[0].instructors[i].groups[j].tutorials)
                labsNum = len(self.courses[0].instructors[i].groups[j].labs)
                ...
                for iTut in range(len(tutsInd)):
                    for iLab in range(len(labsInd)):
                        n = Node(self.courses[0].instructors[i].groups[j],iTut=tutsInd[iTut],iLab=labsInd[iLab])
            ...
            for j in range(len(self.courses[i].instructors)):
                for k in range(len(self.courses[i].instructors[j].groups)):
                    for o in range(len(self.levels[i - 1])):
                        tutsNum = len(self.courses[i].instructors[j].groups[k].tutorials)
                        labsNum = len(self.courses[i].instructors[j].groups[k].labs)
                        tutsInd = list(range(0,tutsNum)) if tutsNum > 0 else [-1]
                        labsInd = list(range(0,labsNum)) if labsNum > 0 else [-1]
                        for iTut in range(len(tutsInd)):
                            for iLab in range(len(labsInd)):
                                ...
                                if not self.levels[i - 1][o].check_clash(self.courses[i].instructors[j].groups[k],
                                                                         self.levels[i - 1][o].schedule,tutsInd[iTut],labsInd[iLab]):
                                    # add the group to the current branch
                                    self.levels[i - 1][o].add_child(self.courses[i].instructors[j].groups[k],tutsInd[iTut],labsInd[iLab])
